...
impl Tag {
    ...
    /// Fetch a tag by name
    pub async fn fetch_by_name(pool: &SqlitePool, tag_name: &str) -> Result<Option<Tag>, sqlx::Error> {
        sqlx::query_as!(
            Tag,
    ...
    pub async fn associate_tag_with_event(pool: &SqlitePool, event_id: i64, tag_id: i64) -> Result<(), sqlx::Error> {
        sqlx::query!(
    ...
    pub async fn fetch_tags_for_event(pool: &SqlitePool, event_id: i64) -> Result<Vec<Tag>, sqlx::Error> {
        let rows = sqlx::query_as!(
            Tag,
            "SELECT t.id, t.name
            FROM tags t
